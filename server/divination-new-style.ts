...
【文字数】500〜600字程度（このパートは重要なので、具体的な描写を充実させてください）
...
/**
 * 第五章を生成（三つの道と具体的なステップ、900〜1,000字）
 */
...
【重要】
- 参考例の「内容」は絶対に真似しないでください
- ${customerInfo.name}さん固有の「三つの道」と具体的なステップを創作してください
...
3. 第三の道の具体的なステップ（600字）
   ...
   **ステップ2：今すぐできる行動**（150字）
   ...
   - 朝、神棚に手を合わせる。
   - 夜、ご先祖様に感謝する。
   - 求人サイトを見る前に、深呼吸して「導いてください」と祈る。
   
   **ステップ3：変化の兆し（6ヶ月後）**（150字）
   - 6ヶ月後。
   - ある日、偶然見かけた求人広告。
   - それは、あなたの経験を必要としている会社からの誘いでした。
   - 「これだ」って、魂が反応する。
...
第五章のみを作成してください。`;
  ...
  const messages = [{ role: 'user' as const, content: prompt }];
  return await callClaudeSonnet(messages, 1500); // max_tokens: 1500 → 約1,050字
}
